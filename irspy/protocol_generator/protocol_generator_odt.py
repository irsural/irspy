...
class _OdfTableFabric:
    ...
    def make(self, data_table: DataTable) -> odfpy_table.Table:
        ...
        for row_index, data_row in data_table.rows:
            ...
            self.__add_row(odf_table, odf_row, row_index)
            self.__curr_cell = 0
    ...
    def __add_row(self, odf_table: odfpy_table.Table, odf_row: odfpy_table.TableRow,
                  pos: int, span: int = 1) -> None:
        for _ in range(self.__curr_row, pos):
            odf_table.addElement(self.__make_odf_row())
...
class OdtProtocolGenerator(ProtocolGenerator):
    ...
    def generate(self, file: Path, config: ProtocolConfig) -> None:
        ...
        for tag, element in self.__find_elements(document, config.tag_map.keys(), self.TEXT_ELEMENT_TYPES):
            value = config.tag_map[tag]
            if isinstance(value, DataTable):
                new_element = self.__table_fabric.make(value)
            else:
                new_element = self.__make_text_element(element, tag, value)
